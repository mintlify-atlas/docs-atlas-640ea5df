...
<ParamField path="--model_name" type="str" default="Qwen/Qwen3-4B-Instruct-2507">
  The model to load or the model ID to send to the API. For local backends this is a Hugging Face Hub model ID or a local path; for API backends it is the model string the provider expects (e.g. `gpt-4o-mini`, `gpt-5.4-mini`).
</ParamField>
